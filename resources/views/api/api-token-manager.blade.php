<div>
    ...
    <x-form-section submit="createApiToken">
        <x-slot name="title">
            {{ __lang('Create API Token') }}
        </x-slot>

        <x-slot name="description">
            {{ __lang('API tokens allow third-party services to authenticate with our application on your behalf.') }}
        </x-slot>

        <x-slot name="form">
            <!-- Token Name -->
            <div class="col-span-6 sm:col-span-4">
                <x-label for="name" value="{{ __lang('Token Name') }}" />
            ...
            @if (Laravel\Jetstream\Jetstream::hasPermissions())
                <div class="col-span-6">
                    <x-label for="permissions" value="{{ __lang('Permissions') }}" />
    ...
    @if ($this->user->tokens->isNotEmpty())
        ...
        <div class="mt-10 sm:mt-0">
            <x-action-section>
                <x-slot name="title">
                    {{ __lang('Manage API Tokens') }}
                </x-slot>

                <x-slot name="description">
                    {{ __lang('You may delete any of your existing tokens if they are no longer needed.') }}
                </x-slot>
                                ...
                                <div class="flex items-center ml-2">
                                    @if ($token->last_used_at)
                                        <div class="text-sm text-gray-400">
                                            {{ __lang('Last used') }} {{ $token->last_used_at->diffForHumans() }}
                                        </div>
                                    @endif
    ...
    <x-dialog-modal wire:model="displayingToken">
        ...
        <x-slot name="content">
            <div>
                {{ __lang('Please copy your new API token. For your security, it won\'t be shown again.') }}
            </div>
    ...
    <x-confirmation-modal wire:model="confirmingApiTokenDeletion">
        <x-slot name="title">
            {{ __lang('Delete API Token') }}
        </x-slot>

        <x-slot name="content">
            {{ __lang('Are you sure you would like to delete this API token?') }}
        </x-slot>
